...
// Global Enumerations:
enum CurrencyConverterCurrency : String, CaseIterable {
    ...
    {
        switch self {
        case CurrencyConverterCurrency.EUR:
            return Currency.EUR
        case CurrencyConverterCurrency.USD:
            return Currency.USD
        case CurrencyConverterCurrency.GBP:
            return Currency.GBP
        default: return .EUR
        }
    }
    ...
    static func nameWithFlag(for currency : CurrencyConverterCurrency) -> String {
        return (CurrencyConverterCurrency.flagsByCurrencies[currency] ?? "?") + " " + currency.rawValue
    }
    ...
    static let allNamesWithFlags : [String] = {
        ...
        for currency in CurrencyConverterCurrency.allCases {
            namesWithFlags.append(CurrencyConverterCurrency.nameWithFlag(for: currency))
        }
        return namesWithFlags
    }()
    
    static let flagsByCurrencies : [CurrencyConverterCurrency : String] = [
        .AUD : "🇦🇺", .INR : "🇮🇳", .TRY : "🇹🇷",
        .BGN : "🇧🇬", .ISK : "🇮🇸", .USD : "🇺🇸",
        .BRL : "🇧🇷", .JPY : "🇯🇵", .ZAR : "🇿🇦",
        .CAD : "🇨🇦", .KRW : "🇰🇷",
...
class CurrencyConverter {
    ...
    // Private Properties:
    private var exchangeRates : [CurrencyConverterCurrency : Double] = [:]
    ...
    /**
     Converts a Double value based on it's currency (valueCurrencyConverterCurrency) and the output currency (outputCurrencyConverterCurrency).
     USD to EUR conversion example: convert(42, valueCurrencyConverterCurrency: .USD, outputCurrencyConverterCurrency: .EUR)
     */
    public func convert(_ value : Double, valueCurrencyConverterCurrency : CurrencyConverterCurrency, outputCurrencyConverterCurrency : CurrencyConverterCurrency) -> Double? {
        exchangeRates = CurrencyConverterLocalData.loadMostRecentExchangeRates()
        guard let valueRate = exchangeRates[valueCurrencyConverterCurrency] else { return nil }
        guard let outputRate = exchangeRates[outputCurrencyConverterCurrency] else { return nil }
    ...
    /**
     ...
     Usage example: convertAndFormat(42, valueCurrencyConverterCurrency: .USD, outputCurrencyConverterCurrency: .EUR, numberStyle: .currency, decimalPlaces: 4)
     */
    public func convertAndFormat(_ value : Double, valueCurrencyConverterCurrency : CurrencyConverterCurrency, outputCurrencyConverterCurrency : CurrencyConverterCurrency, numberStyle : NumberFormatter.Style, decimalPlaces : Int) -> String? {
        guard let doubleOutput = convert(value, valueCurrencyConverterCurrency: valueCurrencyConverterCurrency, outputCurrencyConverterCurrency: outputCurrencyConverterCurrency) else {
            return nil
        }
...
private class CurrencyXMLParser : NSObject, XMLParserDelegate {
    ...
    // Public Methods:
    public func getExchangeRates() -> [CurrencyConverterCurrency : Double] {
    ...
    // Private Methods:
    private func makeExchangeRate(currency : String, rate : String) -> (currency : CurrencyConverterCurrency, rate : Double)? {
        guard let currency = CurrencyConverterCurrency(rawValue: currency) else { return nil }
...
private class CurrencyConverterLocalData {
    ...
    static let fallBackExchangeRates : [CurrencyConverterCurrency : Double] = [
        .USD : 1.1321,
        .JPY : 126.76,
        .BGN : 1.9558,
        .CZK : 25.623,
    ...
    static func saveMostRecentExchangeRates(_ exchangeRates : [CurrencyConverterCurrency : Double]) {
    ...
    static func loadMostRecentExchangeRates() -> [CurrencyConverterCurrency : Double] {
    ...
    /** Converts the [String : Double] dictionary with the exchange rates to a [CurrencyConverterCurrency : Double] dictionary. */
    private static func convertExchangeRatesFromUserDefaults(_ userDefaultsExchangeRates : [String : Double]) -> [CurrencyConverterCurrency : Double] {
        var exchangeRates : [CurrencyConverterCurrency : Double] = [:]
        for userDefaultExchangeRate in userDefaultsExchangeRates {
            if let currency = CurrencyConverterCurrency(rawValue: userDefaultExchangeRate.key) {
    ...
    /**
     Converts the [CurrencyConverterCurrency : Double] dictionary with the exchange rates to a [String : Double] one so it can be stored locally.
     */
    private static func convertExchangeRatesForUserDefaults(_ exchangeRates : [CurrencyConverterCurrency : Double]) -> [String : Double] {
